...
async function initSharedPinia() {
  ...
  try {
    ...
    styleGuide.registerStore("useAdminStore", useAdminStore);

    console.log("[mfe-admin] Shared Pinia initialized and stores registered");
    return sharedPinia;
  } catch (err) {
    console.error("[mfe-admin] Failed to load mfeStyleGuide:", err);
