...
async function initSharedPinia() {
  ...
  try {
    ...
    styleGuide.registerStore("useStudentStore", useStudentStore);

    console.log("[mfe-student] Shared Pinia initialized and stores registered");
    return sharedPinia;
  } catch (err) {
    console.error("[mfe-student] Failed to load mfeStyleGuide:", err);
    // Fallback: cria uma Pinia local
